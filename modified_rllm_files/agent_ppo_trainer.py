...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def init_envs_and_agents(self, batch):
        ...
        if "validate" in batch.meta_info and batch.meta_info["validate"]:
            base_env_args["validation"] = True
        ...
        def _create_agent(i):
            ...
            if self.agent_execution_engine.engine_name == "strands":
                # For strands engine, use the engine's factory method
                return i, self.agent_execution_engine.create_strands_agent(i)
            else:
                # For other engines, use the traditional agent_class
    ...
    def _validate_agent(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # discretize dense reward (e.g. 0.8 => 0), moved to env
            # reward_tensor = (reward_tensor >= 1.0).float() 

            rewards_lst.append(reward_tensor.sum(-1).cpu())
            data_source_lst.append(test_batch.non_tensor_batch.get("data_source", [""] * reward_tensor.shape[0]))
        ...
        for data_source, pass_rates in data_source_uid_pass_rates.items():
            ...
            metric_dict[f"val/test_score/pass@{n_val_samples}/{data_source}"] = np.mean(pass_k_lst)
    ...
    def generate_agent_trajectory(self, timing_raw=None, meta_info=None):
        ...
        with _timer("transform_trajectory", timing_raw):
            ...
            # *** FIX: Clear trajectory data IMMEDIATELY after extraction ***
            # The extracted tensors are now in final_gen_batch_output
            # The original trajectory data is no longer needed
            for traj in trajectories:
                if 'chat_completions' in traj and isinstance(traj['chat_completions'], list):
                    traj['chat_completions'].clear()
                if 'trajectory' in traj and hasattr(traj['trajectory'], 'steps'):
                    traj['trajectory'].steps.clear()
                if 'metrics' in traj and isinstance(traj['metrics'], dict):
                    traj['metrics'].clear()
            
            # Clear the trajectories list itself
            trajectories.clear()
