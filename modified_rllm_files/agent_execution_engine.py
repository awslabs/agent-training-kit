...
try:
    ...
    logger.warning("Strands not available. Install strands-agent to use strands engine.")

try:
    from agentboost.agents.strands_process_pool import StrandsProcessPool
    STRANDS_POOL_AVAILABLE = True
except ImportError:
    STRANDS_POOL_AVAILABLE = False
    logger.warning("StrandsProcessPool not available.")
...
class AgentExecutionEngine:
    def __init__(
        ...
        **kwargs,
        ...
        assert self.engine_name in ["openai", "verl", "strands"], "Currently only openai and verl, strands are supported as rollout engine"
        ...
        elif self.engine_name == "strands":
            if not STRANDS_AVAILABLE:
                raise ImportError("Strands engine requested but strands-agent not installed. Install with: pip install strands-agent")
            if not STRANDS_POOL_AVAILABLE:
                raise ImportError("StrandsProcessPool not available")
                
            self.strands_process_pool = StrandsProcessPool(
                pool_size=kwargs.get('strands_pool_size', 48),
                default_timeout=kwargs.get('strands_timeout', 600)
            )
            
            self.server_addresses = getattr(self.rollout_engine, "server_addresses", [])
            
            self.strands_config = {
                #'base_url': f"http://{self.server_addresses[0]}/v1" if self.server_addresses else "http://localhost:8000/v1",
                'model_id': self.config.actor_rollout_ref.model.path if hasattr(self.config, 'actor_rollout_ref') else "default-model",
                'model_params': {
                    "max_tokens": self.max_response_length,
                    "temperature": self.sampling_params.get("temperature", 0.7),
                    **self.sampling_params
                },
                'tools':  self.agent_args["tools"] or [],
                'system_prompt': self.agent_args["system_prompt"] or self.config.get("strands_system_prompt", "You are a helpful AI assistant.")
            }
    ...
    def create_strands_agent(self, agent_id: int) -> StrandsAgent:
        """Create a new StrandsAgent instance"""
        if self.engine_name != "strands":
            raise ValueError("Can only create StrandsAgent when engine_name is 'strands'")

        config = self.strands_config.copy()
        config['base_url'] = f"http://{self.server_addresses[agent_id % len(self.server_addresses)]}/v1"

        #print(f"Agent {agent_id} base_url: {config['base_url']}")
            
        return StrandsAgent(
            base_url=config['base_url'],
            model_id=self.strands_config['model_id'],
            model_params=self.strands_config['model_params'],
            tools=self.strands_config['tools'],
            system_prompt=self.strands_config['system_prompt'],
            pool=self.strands_process_pool
        )
    ...
    def _extract_user_message_for_strands(self, prompt_messages: list) -> str:
        """Extract user message from prompt_messages for StrandsAgent"""
        ...
        # If no user message found, convert entire prompt to text
        return self.chat_parser.parse(prompt_messages, add_generation_prompt=True, is_first_msg=True)
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        # Handle initial chat_completions differently for StrandsAgent
        if isinstance(agent, StrandsAgent):
            initial_messages = agent.get_initial_system_user_messages(observation)
            ...
            if self.enforce_max_prompt_length and prompt_token_len > self.max_prompt_length:
                agent.reset()
                raise Exception(f"Trajectory {idx}: StrandsAgent initial prompt length {prompt_token_len} exceeded max_prompt_length {self.max_prompt_length}")
        else:
        ...
        for step_idx in range(self.max_steps):
            if isinstance(agent, StrandsAgent):
                ...
                action: Action = await agent.update_from_model("", **kwargs)  # ASYNC
                response = action.action
                
                if isinstance(response, str):
                    if (response.startswith("[CONTEXT_LIMIT_REACHED") or 
                        response.startswith("[TIMEOUT_REACHED")):
                        if "CONTEXT_LIMIT" in response:
                    ...
                    for tool_call in response:
                        if (isinstance(tool_call, dict) and 
                            ...
                            finish_response = tool_call.get("function", {}).get("arguments", {}).get("response", "")
                            if (finish_response.startswith("[CONTEXT_LIMIT_REACHED") or 
                                finish_response.startswith("[TIMEOUT_REACHED")):
                                if "CONTEXT_LIMIT" in finish_response:
                                    termination_reason = "TRUNCATION"
                                else:
                                    termination_reason = "TIMEOUT"
                                break
            ...
            if mode == "Step" and isinstance(response, list):
                assistant_message, _ = get_recent_assistant_user_messages(agent.chat_completions)
                response = assistant_message["content"] if assistant_message else str(response)
            ...
            if done:
                if not termination_reason:  # Don't override existing termination reasons
                    termination_reason = "ENV_DONE"
                break
        ...
        if isinstance(agent, StrandsAgent):
            try:
                ...
                logger.warning(f"Error cleaning up StrandsAgent: {e}")

        if mode == "Text":
            return trajectory
        elif mode == "Token":
            if isinstance(agent, StrandsAgent) and len(response_tokens) == 0:
                logger.warning(f"StrandsAgent trajectory {idx} has empty response_tokens - this may indicate a replay issue")
    ...
    def validate_strands_config(self):
        """Validate strands configuration"""
        if self.engine_name == "strands":
            if not self.server_addresses:
                raise ValueError("Strands engine requires server_addresses")
            
            if not hasattr(self.config, 'actor_rollout_ref'):
                logger.warning("No actor_rollout_ref found in config, using default model")
            
            logger.info(f"Strands engine validated with {len(self.server_addresses)} server(s)")
    ...
    async def trajectory_generator(self, reset_seed=0, timing_raw=None, mode="Text", **kwargs):
        ...
        assert all(agent is not None for agent in self.agents), "All agents must be initialized before trajectory generation"
        ...
        for coro in asyncio.as_completed(tasks_to_run):
            try:
                ...
                colorful_print(f"Skipping trajectory due to exception: {error_msg}", "red")
        ...
        async def sem_wrapper(task_id, task):
            ...
            async with semaphore:
                ...
                try:
                    ...
                    if self.engine_name == "strands":
                        self.agents[index] = self.create_strands_agent(index)
                    else:
