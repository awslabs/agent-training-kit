...
import vllm.entrypoints.openai.tool_parsers.hermes_tool_parser as hermes_parser
from vllm.entrypoints.openai.tool_parsers.hermes_tool_parser import Hermes2ProToolParser
from vllm.entrypoints.openai.tool_parsers.abstract_tool_parser import ToolParserManager
import re
import json

hermes_parser.logger.disabled = True
logger = logging.getLogger(__file__)

CudaCommunicator.all_reduce = all_reduce

def replace_last_match(pattern, replacement_func, text):
    """Replace only the LAST match of a regex pattern in text."""
    ...
    replacement = replacement_func(last_match)
    
    return before + replacement + after

@ToolParserManager.register_module(["hermes_with_sys_prompt"])
class HermesToolParserWithSystemPrompt(Hermes2ProToolParser):
    
    def adjust_request(self, request: ChatCompletionRequest) -> ChatCompletionRequest:
        if not hasattr(self.model_tokenizer, 'chat_template'):
            return request
        
        # Prepend custom system prompt to the system section
        modified_template = self.model_tokenizer.chat_template.replace(
            "{{- '<|im_start|>system\n' }}",
            "{{- '<|im_start|>system\n' }}{% for message in messages %}{% if message.role == 'system' %}{{ message.content + '\n\n' }}{% endif %}{% endfor %}"
        )
    
        self.model_tokenizer.chat_template = modified_template
        return request

# Define this outside any class - it's a standalone utility function
def quick_dump_chat(method: str, request: ChatCompletionRequest):
    """Ultra-minimal request dumper"""
    import os
    import uuid
    from datetime import datetime
    try:
        os.makedirs("/tmp/requests_debug", exist_ok=True)
        filename = f"/tmp/requests_debug/{method}_{datetime.now().strftime('%H%M%S')}_{uuid.uuid4().hex[:6]}.txt"
        with open(filename, 'w') as f:
            for i, msg in enumerate(request.messages):
                f.write(f"{i+1}. {msg.get('role', '')}: {msg.get('content', '')}\n\n")
    except: 
        pass  # Fail silently

def quick_dump_raw_request(method: str, raw_request):
    import os
    import uuid
    from datetime import datetime
    import json
    try:
        os.makedirs("/tmp/requests_debug", exist_ok=True)
        filename = f"/tmp/requests_debug/{method}_{datetime.now().strftime('%H%M%S')}_{uuid.uuid4().hex[:6]}.json"
        with open(filename, 'w') as f:
            json.dump(raw_request, f, indent=4)
    except:
        pass


def quick_dump_completion(method: str, request: CompletionRequest, response: JSONResponse):
    """Ultra-minimal completion request dumper"""
    import os
    import uuid
    from datetime import datetime
    try:
        os.makedirs("/tmp/req_resp", exist_ok=True)
        filename = f"/tmp/req_resp/{method}_{datetime.now().strftime('%H%M%S')}_{uuid.uuid4().hex[:6]}.txt"
        with open(filename, 'w') as f:
            f.write(f"Prompt:\n{request.prompt}\n\n ====================== \n\n")
...
class AsyncvLLMServer(AsyncServerBase):
    ...
    def _patch_vllm_validation(self):
        """Patch vLLM's input validation to handle token limits gracefully"""
        
        serving_engine = self.openai_serving_chat
        original_validate = serving_engine._validate_input
        
        def patched_validate_input(request, input_ids, input_text):
            try:
                return original_validate(request, input_ids, input_text)
            except ValueError as e:
                if "'max_tokens' or 'max_completion_tokens' is too large" in str(e):
                    print(f"⚠️  Token limit exceeded, returning minimal prompt")
        ...
        serving_engine._validate_input = patched_validate_input
        print("✅ Patched _validate_input method")
    ...
    async def init_engine(self):
        ...
        self.openai_serving_chat = OpenAIServingChat(
            ...
            #tool_parser="hermes",
            tool_parser="hermes_with_sys_prompt",
            enable_auto_tools=True
            #return_tokens_as_token_ids=True,
        )
        self._patch_vllm_validation()
    ...
    async def chat_completion(self, raw_request: Request):
        ...
        #quick_dump_chat("chat_completion", request)
        #quick_dump_raw_request("chat_completion", request_json)
        generator = await self.openai_serving_chat.create_chat_completion(request, raw_request)

        if isinstance(generator, ErrorResponse):
            print(f"ERROR RESPONSE DETAILS:")
            status_code = getattr(generator, 'code', 500)
    ...
    async def completions(self, raw_request: Request):
        ...
        if request.stream:
            ...
            #quick_dump_completion("completions", request, response)
            return response
    ...
    async def chat_completion_generator(self, request: ChatCompletionRequest) -> AsyncGenerator[Tuple[int, str]]:
        ...
        """
        #quick_dump_chat("chat_completion_generator", request)
